...
import {getEmbedData} from "./utils/api";
...
const selectEmbed = ({ className, isSelected, attributes, setAttributes }) => {
	const [embedData, setEmbedData] = useState([]);
	const [embedHtml, setEmbedHtml] = useState(__('Loading...', 'simplybook') );
	const [dataLoaded, setDataLoaded] = useState(false);

	useEffect(() => {
		api.getEmbedData().then( ( response ) => {
	...
	if ( attributes.preview ) {
		return(
			<img alt="preview" src={simplybook.preview} />
		);
	}

	return [
		<div className={className} dangerouslySetInnerHTML={{__html: DOMPurify.sanitize( embedHtml ) }}></div>,
	]
}
...
registerBlockType('simplybook/embed', {
	title: __('Bookings', 'simplybook'), // Block title.
	...
	keywords: [
		__('Booking, Reservation system, Meeting', 'simplybook'),
	],
